...
function main() {
  ...
  const ctx = {
    ...
    shadow_material: {
      ...
      blend: blend_shadow,
      attrib_stride: 12,
      attribs: [{ name: 'a_vertex', size: 3, offset: 0 }],
    },
  };
...
function draw(ctx, time) {
  ...
  const rectangle_transform = mat4_trs(
    0.3 * Math.cos(time),
    0.3 * Math.sin(time),
    time,
    1
  ...
  for (var i in lights) {
    ...
    bind_material(gl, ctx.light_material, [
      { name: 'u_color', type: UNIFORM.vec3, value: light.color },
      {
        name: 'u_matrix',
        type: UNIFORM.mat4,
        value: mat4_trs(light.x, light.y, 0, light.size),
      },
    ]);
...
function create_shader(gl, LIGHT_VSHADER, LIGHT_FSHADER) {
  ...
  if (!gl.getProgramParameter(shader, gl.LINK_STATUS)) {
    alert(
      'Unable to initialize the shader shader: ' + gl.getProgramInfoLog(shader)
    );
...
const UNIFORM = {
  vec2: function (gl, loc, value) {
    gl.uniform2fv(loc, value);
  },
  vec3: function (gl, loc, value) {
    gl.uniform3fv(loc, value);
  },
  mat4: function (gl, loc, value) {
...
function bind_material(gl, material, uniforms) {
  ...
  for (var i in uniforms) {
    ...
    uniform.type(
      gl,
      gl.getUniformLocation(material.shader, uniform.name),
      uniform.value
    );
  }
}
